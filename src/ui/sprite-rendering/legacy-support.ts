...
import { IRendererData, IRendererParam, IRenderConfiguration } from "./renderer";
...
export function ConvertLegacyConfiguration(
    ...
    return ({
        ...
        params: renderData.map(data => <IRendererParam>{
            name: data.name,
...
export function ConvertLegacySpritesToRenderData(
    ...
    return Object.keys(sprites).map(key => (<IRendererData>{
        name: key,
        paramInfo: {
            min: 0,
            max: 1,
