...
export type AudioQueue = {
  ...
  QueueAudioBufferList?: (
    audioBuffers: (AudioBuffer | Promise<AudioBuffer>)[]
  ) => void;
  QueueAudioBuffer: (audioBuffer: AudioBuffer | Promise<AudioBuffer>) => void;
...
export async function CreateAudioQueue(
  ...
  const audioQueue: AudioQueue = {
    ...
    analyzer,
    sourceNodes,
    QueueAudioBufferList(audioBuffers: (AudioBuffer | Promise<AudioBuffer>)[]) {
      audioBuffers.forEach((audioBuffer) => {
        audioQueue.QueueAudioBuffer(audioBuffer);
      });
    },
    QueueAudioBuffer(audioBuffer: AudioBuffer | Promise<AudioBuffer>) {
      if (audioBuffer instanceof Promise) {
        ...
        audioBufferSourceNode.connect(analyzer);
        sourceNodes.push(
          new Promise<AudioBufferSourceNode>((resolve) =>
    ...
    async PlayQueue() {
      while (sourceNodes.length > 0) {
        try {
          ...
          sourceNodes[0]?.then((sourceNode) => console.log(sourceNode.buffer));
          console.error(error);
        }
        sourceNodes.shift();
      }
    ...
    GetFrequencyData(
      ...
      for (let i = 0; i < length; i++) {
        let low = 0;
        if (logScale) {
          low = Math.sqrt(i / length) * byteDataLength;
        } else {
        ...
        data[i] = byteData.slice(low, high).reduce((a, b) => a + b, 0);
        data[i] /= (high - low) * 256;
      }
      return data;
    },
