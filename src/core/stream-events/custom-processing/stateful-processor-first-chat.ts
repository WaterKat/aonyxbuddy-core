...
/**
 * TODO Temporary fix, will be replaced with a more robust solution in the future
 */
export function LoggedStatefulProcessFirstChat(): (
  event: TStreamEvent
) => Logger<TStreamEvent> {

  let state: { chatters: string[] } = {
    chatters: [],
  };

  return function (event: TStreamEvent): Logger<TStreamEvent> {
    const newEvent = StatefulProcessFirstChat({
      event: event,
      options: {},
      state: state,
    });

//   console.log("old", state, "new",newEvent.state);

    state = newEvent.state;

    return new Logger(newEvent.event).log(
      `first message from ${newEvent.event.username}`
    );
  };
}
