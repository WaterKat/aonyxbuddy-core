...
import {
  GetFilterBlacklistFunction,
  FilterBlacklistOptions,
  GetProcessCommandFunction,
  ProcessCommandOptions,
  GetFilterBotlistFunction,
  FilterBotlistOptions,
  GetFilterCaseSensitiveFunction,
  FilterWordsOptions,
  GetFilterCaseInsensitiveFunction,
  GetFilterEmojisFunction,
  FilterEmojisOptions,
  GetFilterCheermotesFunction,
  FilterCheermotesOptions,
  GetValidateFunction,
  ValidateOptions,
  GetFilterPermissionsFunction,
  FilterPermissionsOptions,
  GetNicknamesFunction,
  NicknamesOptions,
...
import {
  LoggedStatefulProcessFirstChat,
  LoggedStatefulProcessIgnoreRaid,
} from "./custom-processing/index.js";
...
//  const filterCondition = GetFilterConditionFunction(options.conditionOptions);
  ...
  const filterCaseSensitive = GetFilterCaseSensitiveFunction(
    options.caseSensitiveOptions
  );
  const filterCaseInsensitive = GetFilterCaseInsensitiveFunction(
  ...
  const filterPermissions = GetFilterPermissionsFunction(
    options.permissionsOptions
  );

  const detectFirstChat = LoggedStatefulProcessFirstChat();
  const ignoreEventsAfterRaid = LoggedStatefulProcessIgnoreRaid();

  return function (event: TStreamEvent) {
    const loggedEvent = new Logger(event)
      .log("ProcessEvent:\n")
      ...
      .log("FilterBotlist: ")
      .map(filterBotlist)
      .log("ProcessCaseSensitive: ")
      .map(filterCaseSensitive)
      .log("ProcessCaseInsensitive: ")
      .map(filterCaseInsensitive)
      .log("FilterEmojis: ")
      .map(filterEmojis)
      .log("FilterCheermotes: ")
      .map(filterCheermotes)
      .log("GetNicknames: ")
      .map(getNicknames)
      .log("FilterPermissions: ")
      .map(filterPermissions)

      .log("StatefulProcessFirstChat: ")
      .map(detectFirstChat)
      .log("StatefulProcessIgnoreRaid: ")
      .map(ignoreEventsAfterRaid);
