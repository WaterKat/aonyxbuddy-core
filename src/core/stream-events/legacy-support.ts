...
import { EPermissionLevel as EPermission } from "./processing/filter-permissions.js";
...
export function IsLegacyEventProcessorConfig(
  ...
  if (!ObjectContainsKey(value, "owner_id")) return false;
  if (typeof value.owner_id !== "string") return false;
  if (!ObjectContainsKey(value, "nickname")) return false;
  if (typeof value.nickname !== "string") return false;
  if (!ObjectContainsKey(value, "nicknames")) return false;
  if (typeof value.nicknames !== "object") return false;
  if (!ObjectContainsKey(value, "blacklist")) return false;
  if (typeof value.blacklist !== "object") return false;
  if (!ObjectContainsKey(value, "botlist")) return false;
  if (typeof value.botlist !== "object") return false;
  if (!ObjectContainsKey(value, "blockedWords")) return false;
