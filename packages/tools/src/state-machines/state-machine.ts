...
export class AnimationStateManager {
    ...
    private async Update() {
        ...
        if (this.transitionRequests.length > 0) {
            ...
            const awaitedAnimationPromise: IAnimationState | null = await animationPromise ?? null;
    ...
    private async GetDirectedTransition(requestedStateID: string, originID: string): Promise<IAnimationState | null> {
        ...
        while (transitions.length > 0) {
            ...
            if (isEndpoint) {
                ...
                let backtrackingTransitions: IAnimationTransition[] = [workingTransition];
                let checkedBacktrackingTransitions: IAnimationTransition[] = [];
