...
export class PNGRenderer implements IAsyncInitializable, IRenderer {
    ...
    async fetchImageData(_pngConfig: PNGConfig.IPNGConfig) {
        for (let key in _pngConfig.sprites) {
    ...
    async getStatesFromImageData() {
        for (let key in this.config.sprites) {
    ...
    getTransitions() {
        for (let transitionKey in this.config.transitions) {
            for (let key in this.config.transitions[transitionKey]) {
