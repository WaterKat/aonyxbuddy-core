...
import { WebSockets } from "./external";
import { StreamCharacters, StreamEvents, Tools } from './external';
...
async function GetEventClient(blockedText: string[], botBlacklist : string[], nicknames: StreamEvents.Clients.PostProcessors.Nickname.INicknamePostProcessorOptions): Promise<StreamEvents.Clients.EventManager> {
    ...
    const wsListener = {
        eventSubscription : new Tools.Subscription<StreamEvents.StreamEvent>()
    }
    ...
    //remove bad words
    const textFilterPostProcessor = StreamEvents.Clients.PostProcessors.TextFilterPostProcessor(blockedText, false, 'EEEE');
    client.attachPostProcessor(textFilterPostProcessor);

    //convert chat messages into command event
    const commandPostProcessor = StreamEvents.Clients.PostProcessors.CommandPostProcessor();
    client.attachPostProcessor(commandPostProcessor);

    //blacklist chat messsages
    const blacklistPostProcessor = new StreamEvents.Clients.PostProcessors.BlackList.BlackListPostProcessor(botBlacklist);
    client.attachPostProcessor(blacklistPostProcessor);

    //convert command say to custom event
    const commandSayPostProcessor = new StreamEvents.Clients.PostProcessors.CommandSay.CommandSayPostProcessor();
    client.attachPostProcessor(commandSayPostProcessor);

    //remove commands without permissions
    const permissionPostProcessor = new StreamEvents.Clients.PostProcessors.Permissions.SkipEventCommandPostProcessor();
    client.attachPostProcessor(permissionPostProcessor);

    //nicknames
    const nicknamePostProcessor = new StreamEvents.Clients.PostProcessors.Nickname.NicknamePostProcessor(nicknames);
    client.attachPostProcessor(nicknamePostProcessor);

    //mute command post processor
    const muteCommandPostProcessor = new StreamEvents.Clients.PostProcessors.MuteCommandPostProcessor();
    client.attachPostProcessor(muteCommandPostProcessor);

    //skip command post processor
    const skipCommandPostProcessor = new StreamEvents.Clients.PostProcessors.SkipEventCommandPostProcessor();
    client.attachPostProcessor(skipCommandPostProcessor);

    //remove emojis
    const emojiFilterPostProcessor = StreamEvents.Clients.PostProcessors.EmojiPostProcessor();
    client.attachPostProcessor(emojiFilterPostProcessor);

    //raid timeout for messages and follows
    const raidIgnorePostProcessor = new StreamEvents.Clients.PostProcessors.RaidIgnorePostProcessor();
    ...
    const firstMessagePostProcessor = new StreamEvents.Clients.PostProcessors.FirstEventPostProcessor();
    ...
    const subscriberPostProcessor = new StreamEvents.Clients.PostProcessors.Subscriber.SubscriberPostProcessor();
...
async function GetCharacter(
    ...
    const renderer = await Tools.AsyncClassInitializer.create(StreamCharacters.PNG.PNGRenderer, pngconfig);

    if (!renderer)
        throw (renderer);

    const tts = new StreamCharacters.TextToSpeech.StreamElementsTTS.StreamElementsTTS(ttsconfig);
    ...
    const character = await Tools.AsyncClassInitializer.create(StreamCharacters.StreamCharacters.StreamCharacter, streamCharacterOptions);
...
async function main() {
    ...
    const character = await GetCharacter(ClientConfigExample.png, ClientConfigExample.tts);
    const eventClient = await GetEventClient(ClientConfigExample.badWords, ClientConfigExample.botBlacklist, ClientConfigExample.nicknames);
    ...
    const eventParser = new StreamEvents.Parsers.StreamEventParser(eventParserOptions);
